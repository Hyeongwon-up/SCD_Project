...
public class huff {
	...
	// lang", "math", "collections", "ivy","hadoop", "hbase","derby"

	public static String[] projects = { "lang" };
	...
	public static void main(String[] args) {
		...
		try {
			...
			PreparedStatement psIns = con.prepareStatement(
					"insert into changes_ver3 ( fileId, tool, change_type, entity_type, edit ) values ( ?, ?, ?, ?, ? )");
			...
			for (int x = 0; x < projects.length; x++) {
				...
				PreparedStatement ps = con
						.prepareStatement("insert into files_ver2(project, issue_id, old_commitid, new_commitid, file) "
								+ "values ('" + project + "', ?, ?, ?, ?)");
				...
				for (int i = 0; i < commitIds.size(); i++) {
					...
					for (int j = 0; j < files.size(); j++) {
						...
						try {
							...
							String oldVer = oldFile.getPath();
							String newVer = newFile.getPath();
							/*
							 * Runtime rt = Runtime.getRuntime(); StringBuffer HW = new StringBuffer();
							 * String result = ""; List<String> cmd = new ArrayList<String>();
							 * cmd.add("diff -y --suppress-common-lines"); cmd.add("-y");
							 * cmd.add("--suppress-common-lines"); cmd.add(oldVer); cmd.add(newVer);
							 * cmd.add("|"); cmd.add("wc"); cmd.add("-l"); String joined = String.join(" ",
							 * cmd); String[] cmd2 = { "/bin/sh", "-c", joined };
							 * 
							 * try { Process proc = rt.exec(cmd2); BufferedReader in = new
							 * BufferedReader(new InputStreamReader(proc.getInputStream())); String cl =
							 * null; while ((cl= in.readLine()) != null) { HW.append(cl); } result =
							 * HW.toString();
							 * 
							...
							if (tool == GUMTREE) {
								...
								for (SourceCodeChange c : changes) {
									...
									// psIns.setString(3, c.getChangeType().toString());
									psIns.setString(4, c.getChangedEntity().toString());
